...
import { categoryOptions, getSubcategories } from "./_data/categories";
...
export default function MarketPage() {
  ...
  }, [products, selectedCategory, selectedSubcategory, locationQuery]);

  const subcategoryOptions = useMemo(() => {
    return selectedCategory === "all" ? [] : getSubcategories(selectedCategory);
  }, [selectedCategory]);

  // Calculate total items saved from waste
  const totalSaved = useMemo(() => {
    return products.reduce((acc, p) => acc + (p.ecoSaved || 1), 0);
  }, [products]);
