...
import { useProductImageUpload, createProduct } from "@/logic/products.logic";
...
function SellFormContent() {
  ...
  const handleImageChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const uploadError = await addImages(selectedFiles);
    if (uploadError) setError(uploadError);
    if (!uploadError && selectedFiles.length > 0) setError(null);
    event.target.value = "";
  };
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  <label className="cursor-pointer rounded-2xl border border-emerald-200 bg-white px-4 py-3 text-center text-sm font-bold text-emerald-700 transition hover:bg-emerald-50">
                    ...
                    <input
                      ...
                      accept="image/*"
                      multiple
                      disabled={imageLoading || imageUrls.length >= maxPhotos}
